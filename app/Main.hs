...
darkTheme =
    V.Attr
        ...
        , V.attrForeColor = V.SetTo V.white
...
data Board = Board
    { boardVals :: [[Maybe Bool]]
    , boardRowHints :: [[Word8]]
    , boardColHints :: [[Word8]]
    }
instance Show Board where
    show (Board v r c) =
        let boardRows = concatMap (" " <>) . fmap showSquare <$> v
            rowHints = showRowHint <$> r
            header = showColHints c
            body = replicate 30 '_' : zipWith (\r v -> r <> ('|' : v)) rowHints boardRows
         in unlines . leftPad $ header <> body
        where
            maxLength :: [[a]] -> Int
            maxLength = maximum . fmap length
            showSquare = maybe "_" (bool "×" "■")
            showRowHint :: [Word8] -> String
            showRowHint = intercalate "," . fmap (\i -> if i < 10 then ' ' : show i else show i)
            leftPad :: [String] -> [String]
            leftPad l = fmap (\s -> iterate (' ' :) s !! (maxLength l - length s)) l
            showColHints :: [[Word8]] -> [String]
            showColHints cs =
                let padded = fmap (maybe "  " $ \i -> if i < 10 then ' ' : show i else show i) <$> topPad cs
                    folded = foldr (zipWith (<>)) (replicate 15 []) padded
                 in folded
            topPad :: [[a]] -> [[Maybe a]]
            topPad l = fmap ((\s -> iterate (Nothing :) s !! (maxLength l - length s)) . fmap Just) l


newtype Solution = Solution {solutionVals :: [[Bool]]}
instance Show Solution where
    show (Solution sol) = unlines $ intercalate " " . fmap showSquare <$> sol
        where
            showSquare True = "■"
            showSquare False = "×"


newSolution :: IO Solution
newSolution = Solution <$> replicateM 15 (replicateM 15 randomIO)


newBoard :: Solution -> Board
newBoard (Solution sol) =
    let boardRowHints = hints <$> sol
        boardColHints = hints <$> transpose sol
     in Board (replicate 15 (replicate 15 Nothing)) boardRowHints boardColHints
    where
        hints :: [Bool] -> [Word8]
        hints = uncurry (<>) . first maybeToList . foldr go (Nothing, [])
        go :: Bool -> (Maybe Word8, [Word8]) -> (Maybe Word8, [Word8])
        go square (currHint, hintStack) = case (square, currHint) of
            (False, Nothing) -> (Nothing, hintStack)
            (False, Just n) -> (Nothing, n : hintStack)
            (True, Nothing) -> (Just 1, hintStack)
            (True, Just n) -> (Just (n + 1), hintStack)
            ...
            let escapable w = do
                    ...
                    return $
                        fforMaybe i $ \case
                            ...
                            _ -> Nothing
            rec out <- networkHold buttons $
                    ffor (switch (current out)) $ \case
                        ...
                        (a3, b3, c3) <-
                            grout (fixed 3) $
                                row $ do
                                    ...
                                    return (a, b, c)
                        pure ((a1, b1, c1), (a2, b2, c2), (a3, b3, c3))
                tile (fixed 7) $
                    boxTitle (constant def) "CLICK BUTTONS TO DRAW*" $ do
                        outputDyn <-
                            foldDyn (<>) "" $
                                mergeWith
                                    (<>)
...
dragTest = do
    lastEvent <- hold "No event yet" . fmap show =<< drag V.BMiddle
    text $ T.pack <$> lastEvent
